...
const Login = () => {
  ...
  const handleSocialAuth = (provider) => {
    alert(`${provider} Sign-in option selected. Logging in...`);
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 font-sans py-12">
      ...
      <div className="w-full max-w-sm space-y-6 relative z-10">
        ...
        {/* Login Type Tabs / Portal Switcher */}
        <div className="flex bg-white/80 p-1.5 rounded-2xl border border-slate-200 shadow-xs">
          <button
            type="button"
            className="flex-1 py-2 text-xs font-bold rounded-xl bg-blue-600 text-white shadow-xs text-center"
          >
            User Login
          </button>
          <Link
            to="/admin/login"
            className="flex-1 py-2 text-xs font-semibold rounded-xl text-slate-600 hover:text-slate-900 text-center transition-all flex items-center justify-center gap-1.5"
          >
            <Lock className="h-3.5 w-3.5 text-blue-600" /> Admin Portal
          </Link>
        </div>
        ...
        <div className="bg-gradient-to-b from-white to-slate-50/90 rounded-[32px] p-7 md:p-8 border-4 border-white shadow-[0_30px_30px_-20px_rgba(133,189,215,0.7)] space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1">
              <div className="relative">
                <input
                  ...
                  placeholder="E-mail"
                  className="w-full bg-white border border-slate-200/80 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-400/20 px-5 py-3.5 rounded-2xl text-slate-800 placeholder-slate-400 text-sm outline-none transition-all shadow-[0_10px_10px_-5px_#cff0ff]"
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="text-left pl-2 pt-1">
                <button
                  type="button"
                  onClick={() => alert('Reset link instructions sent to your email address.')}
          ...
          <div className="space-y-1.5 text-center pt-1">
            <p className="text-xs font-semibold text-slate-500">
              ...
              <Link to="/register" className="font-bold text-blue-500 hover:text-blue-600 hover:underline">
                Create an account
              </Link>
            </p>
            ...
            <button
              type="button"
              onClick={() => alert('User Licence Agreement: By logging into HackPilot, you agree to platform security and terms of service.')}
